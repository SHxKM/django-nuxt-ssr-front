<template>
    <div>
        ...
        <p>
            <b>You have
                <transition name="fade" mode="out-in"><span :key="todos_counter">{{ todos_counter }}</span>
                </transition> <span v-if="todos_counter !== 1">items</span><span v-else>item</span>
            </b>.
        </p>
        <p>
            Watch how this entirely independent component utilizes a <code>Vuex</code> store (state) in order to update the todo counter whenever a change ocurrs.
        </p>
        ...
        <div class="bg-yellow-200 rounded p-3 border-yellow-400 border-2 max-w-xs">
            <p class="text-yellow-700">
                ...
                dyno <a href="https://devcenter.heroku.com/articles/free-dyno-hours#dyno-sleeping" target="_blank">that
                sleeps</a> after 30 minutes of inactivity. Therefore it could load much slower than it's supposed to
