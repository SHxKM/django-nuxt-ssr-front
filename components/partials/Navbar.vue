<template>
  <nav class="flex items-center justify-between flex-wrap bg-teal-500 p-6">
    <div class="flex items-center flex-shrink-0 text-white mr-6">
      <nuxt-link class="font-semibold text-white hover:text-white no-underline text-base lg:text-xl tracking-tight" :to="{'name': 'index'}">Django Nuxt SSR - To Do With Vue</nuxt-link>
    </div>
    ...
    <div :class="burgerToggled ? 'block': 'hidden'" class="w-full  flex-grow lg:flex lg:items-center lg:w-auto">
      <div class="text-sm lg:flex-grow">
        ...
        :to="{'name': 'index'}" @click.native="hideBurger"
        >
        Home
      </nuxt-link>
      <nuxt-link
      class="block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4"
      :to="{'name': 'about'}" @click.native="hideBurger"
      >
      About
    </nuxt-link>
    <a
